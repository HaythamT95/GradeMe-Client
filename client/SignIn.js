...
const SignIn = ({ navigation }) => {
    ...
    const handleSubmit = async () => {
        ...
        const resp = await axios.post(`http://${HOST}:8000/api/signin`, { email, password }).catch(err=>err);
        if(resp.data.error)
        ...
        <KeyboardAwareScrollView contentCotainerStyle={styles.container}>
            <View style={{ marginVertical: 100 }}>
                <View style={styles.imageContainer}>
                    <Image source={require("../assets/logo.jpg")} style={styles.imageStyles} />
                </View>
                ...
                <Text style={{ fontSize: 12, textAlign: 'center' }}>Not yet registered?{" "}
                    <Text style={{ color: "darkred", fontWeight: "bold" }} onPress={() => navigation.navigate("SignUp")}>Sign Up
                    </Text>
                </Text>
